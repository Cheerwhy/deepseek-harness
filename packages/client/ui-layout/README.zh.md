...
本包提供 Web GUI 的三栏 AppFrame、左右栏宽度与 `ctx.layout` 呈现控制。右栏先让步以保护中栏空间，全屏由占用方呈现，框架保留宽屏底层轨道。主题呈现器负责配色、别名 token、正文字号与 document 元数据；两栏宽度偏好在刷新后保留，其余布局状态重置。
...
本插件在 root slot 中组合侧边栏、主内容和右栏。侧边栏宽度为 264～420px，默认为 280px，收起后保留 56px 控制栏；窗口宽度低于 1024px 时自动收起，打开右侧面板也会收起手动展开的侧边栏。右侧面板首次打开时使用视口宽度的 45%，之后保留用户的像素宽度偏好，上限为 70%。两栏宽度偏好持久化在同一个 localStorage 条目（`dsh.layout.widths.v1`）中，刷新后保留，首次打开的 45% 默认值同样计入；收起的侧边栏刷新后保持收起，刷新后再展开仍从 280px 默认值开始。为给中栏保留 400px，框架先将右侧面板缩减至 300px，再报告空间不足，使占用方将其关闭，最后才进一步压缩中栏。拖动没有过渡延迟；右侧手柄在关闭或全屏时不显示。
...
- **宽度偏好按框架持久化，不按会话**——两栏宽度偏好（含收起的侧边栏）刷新后从同一个 `dsh.layout.widths.v1` localStorage 条目恢复；框架测量、响应式覆写与占用方呈现报告仍是瞬时状态。
- **极窄窗口**——右侧面板关闭后，中栏仍可能小于 400px；左侧 56px 控制栏仍会保留。
- **轨道与面板仅在动画期间沿同一条曲线运动**——离散开合时框架在渲染新列宽的同时设置 `data-animating`，轨道过渡和占用方的滑入读取同一组时长与缓动变量。拖拽和即时呈现切换不带过渡，因此该曲线不覆盖它们；占用方在刷新或切换会话后的首次上报带恢复标记，其轨道直接安装、不走该曲线。
