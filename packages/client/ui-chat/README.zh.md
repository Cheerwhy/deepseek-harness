...
使用本包可渲染已记录的 Session 对话，支持历史图片和滚动位置恢复。工作步骤展示模式控制思考预览与过程显隐，不隐藏最终答案；经典模式在最终答案和完整历史加载后收起已完成轮次的过程行。本地 transcript（文本记录）与 steering（中途引导）提交会立即显示并保留在原区域，在权威会话记录到达时原子地消失，而排队中的提交始终不进入 Chat。本包不组装或修改模型请求。
...
通用指令行在所有生命周期状态中都保留普通指令图标，运行时只沿整行显示一次扫光；失败仍通过行状态与摘要明确表达。终止轮次的错误仍是独立的红点提示；模型的中间重试不会创建该提示，达到输出 token 上限时使用琥珀色警告点。
...
工作步骤展示提供经典（默认）、简洁、标准、详细和完全展开。经典模式在最终答案和完整历史加载后收起已完成轮次的过程行，以数量摘要作为展开控件。简洁、标准、详细模式收起符合条件的已完成轮次，不隐藏最终答案；完全展开模式保留时长或状态抬头，但不支持收起。轮次运行时，蓝色活动文字显示在已写入会话的内容之后、待处理插话回显之前。[业务规则明细](src/client/conversation-nodes/README.zh.md#display-modes) 统一说明模式表、组头行为、整轮折叠资格、时钟与开合重置。
