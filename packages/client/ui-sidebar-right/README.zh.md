...
普通与全屏共用同一棵面板内容树，切换不会重挂载Tab。普通面板贴靠右栏；全屏面板覆盖窗口并保留宽屏底层列宽。窗口低于768px时打开右栏自动全屏；窄屏退出全屏会收起右栏，变宽不重新打开已关闭的右栏。 全屏打开时，底层列宽保持不变，直到滑入结束后才无过渡地准备普通轨道。 全屏面板退场前，关闭先准备全宽会话区，恢复先准备普通右轨道；退场期间底层不播放宽度动画。 任何打开路径都让面板沿框架的曲线滑入：初次打开与关闭全部标签后的再打开会全新插入滑入的停靠内容，单靠过渡没有起始状态，因此打开翻转会短暂标记面板，只有该标记在该曲线上携带一段入场动画；已开面板内的结构变化——拆分、悬浮、切换标签——不携带标记，保持无动画。 seat 挂载后的首次上报——刷新或切换会话时恢复持久化的展开状态——带恢复标记，框架直接落位轨道，而不是在用户从未打开的面板上缓动中栏。
...
- **关闭最后一个标签时面板瞬间消失。** 替换宿主的空停靠内容在插入时已是收起态，没有可见内容可以滑出；要滑出需要延迟卸载。
